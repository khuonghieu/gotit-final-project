...
import DeleleItemModal from './DeleteItem';
...
ModalMap.set(constants.DELETE_ITEM_MODAL, DeleleItemModal);
...
export function ModalContainer({ modal, closeModal }) {
  ...
  return (
    <div>
      {modal.modalChosen ? <ModalToRender onClose={closeModal} prefill={modal.prefill} callbackFunc={modal.callbackFunc} /> : null}
    </div>
  );
}
